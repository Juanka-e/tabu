...
const economySettingsSchema = z.object({
    ...
    matchRewardGuardEnabled: z.boolean().default(false),
    matchRewardWindowHours: z.number().int().min(1).max(168).default(24),
    matchRewardSoftCapCoin: z.number().int().min(0).max(100_000).default(3000),
    matchRewardHardCapCoin: z.number().int().min(0).max(100_000).default(5000),
    matchRewardMinMultiplier: z.number().min(0).max(1).default(0.25),
    matchRewardDampingProfile: z.enum(ECONOMY_DAMPING_PROFILES).default("standard"),
    repeatedGroupEnabled: z.boolean().default(false),
    repeatedGroupWindowHours: z.number().int().min(1).max(168).default(12),
    repeatedGroupThreshold: z.number().int().min(2).max(100).default(6),
    repeatedGroupMinMultiplier: z.number().min(0).max(1).default(0.4),
});
